...
Early versions faced a raced condition when using an `iptables` forwarding rule: the original network packet leaked through to the true router. The client device would receive two conflicting answers, triggering a browser network crash or an unresolvable `NXDOMAIN` error.

The solution implemented in this architecture relies on a strict kernel-to-userland boundary handoff:
1. `iptables` intercepts incoming UDP port 53 traffic and forwards it to Linux kernelspace via **`NFQUEUE`**.
